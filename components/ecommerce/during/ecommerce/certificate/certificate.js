...
const isQADomain = () => {
    return (
        window.location.host === "qa.goemms.com" ||
...
const getUrlWorkshop = () =>
    WORKSHOPS.find(workshop => window.location.href.includes(workshop)) ||
    false;
...
const forceDownload = async (fullname, type) => {
    ...
    const domainUrl = isQADomain()
        ? `certificate-emms2025qa.php`
        : `certificate-emms2025.php`;
    const commonUrl = `https://textify.fromdoppler.com/${domainUrl}?fullname=${encodeFullname}&type=${type}`;
    const url = workshopType
        ? `${commonUrl}&workshoptype=${encodeURI(workshopType)}`
        : commonUrl;
...
const submitCertificateWithoutForm = async (
    e,
    type,
    submitButton,
    userName
