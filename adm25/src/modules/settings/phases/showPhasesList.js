...
export const showPhasesListPage = async () => {
    ...
    const digitalTrendsForm = document.getElementById(
        "digital-trends_current_phase"
    );
    digitalTrendsForm.addEventListener("submit", function (e) {
        sendDataCurrentPhase(e, "digital-trends");
    });

    const ecommerceTransmissionForm = document.getElementById(
        "ecommerce_transmission"
    );
    ecommerceTransmissionForm.addEventListener("submit", function (e) {
        sendDataCurrentTransmission(e, "ecommerce");
    });

    const digitalTrendsTransmissionForm = document.getElementById(
        "digital-trends_transmission"
    );
...
function hideAlerts() {
    ...
    const ecoLive = document.getElementById(
        "ecommerce_transmission-alert-success"
    );
    ecoLive.style.display = "none";
    const dtLive = document.getElementById(
        "digital-trends_transmission-alert-success"
    );
...
const sendDataCurrentPhase = async (e, event) => {
    const selectedPhase = document
        .querySelector('input[name="' + event + '_phase"]:checked')
...
const checkRadiosPhase = async event => {
    ...
    document.getElementById(event + "_" + objResult.current_phase).checked =
        true;
...
const sendDataCurrentTransmission = async (e, event) => {
    ...
    const transmission = document.querySelector(
        'input[name="' + event + '_transmission"]:checked'
    ).value;
...
const checkRadiosTransmission = async event => {
    const objResult = await getPhase(event);
    document.getElementById(event + "_" + objResult.transmission).checked =
        true;
};
