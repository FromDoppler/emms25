        ...
        if (element.id.length !== 0) {
            if (element.type === "file") {
                if (
                    ...
                    myObject[element.id] = getRandomFileName(
...
export const showSponsorForm = async (currentSponsorType, objSponsor) => {
    const sponsorsFormComponentUrl =
        "src/modules/sponsors/components/sponsor-form/sponsor-form.html";
    const response = await fetch(sponsorsFormComponentUrl);
    const app = document.querySelector("app");
    app.innerHTML = await response.text();
    app.querySelector("#sponsorTitle").innerHTML +=
...
const removeFieldsBySponsorType = currentSponsorType => {
    id_field_col_array.forEach(element => {
        if (element.hiddenType.includes(currentSponsorType))
            document.getElementById(element.col).remove();
    });
};
